...
Status Executor::Execute(const Statement& stmt, std::ostream& out) {
    switch (stmt.GetType()) {
        ...
        case StatementType::EXPLAIN:
            return ExecuteExplain(static_cast<const ExplainStatement&>(stmt), out);
        default:
...
Status Executor::ExecuteInsert(const InsertStatement& stmt, std::ostream& out) {
    ...
    for (const auto& fk : schema.GetForeignKeys()) {
        ...
        if (!found) return Status::IOError("Foreign Key constraint violation: parent key not found in " + fk.referenced_table);
    }
...
Status Executor::ExecuteSelect(const SelectStatement& stmt, std::ostream& out) {
    ...
    if (!used_index) {
        ...
        for (auto& rec : all_records) {
            if (Matches(rec, schema_left, stmt.where.get())) records.push_back(std::move(rec));
        }
    }

    if (!stmt.aggregates.empty()) {
        struct AggState { double sum = 0; int64_t count = 0; double min = 1e18; double max = -1e18; };
        std::map<std::string, std::vector<AggState>> groups;
        size_t group_by_idx = 0; bool has_group_by = false;
        if (!stmt.group_by_column.empty()) {
            has_group_by = true;
            for (size_t i = 0; i < schema_left.GetColumnCount(); ++i)
                if (schema_left.GetColumn(i).GetName() == stmt.group_by_column) { group_by_idx = i; break; }
        }
        for (const auto& rec : records) {
            ...
            for (size_t i = 0; i < stmt.aggregates.size(); ++i) {
                const auto& agg = stmt.aggregates[i]; double val = 0;
                if (agg.column_name != "*") {
                    for (size_t c = 0; c < schema_left.GetColumnCount(); ++c)
                        if (schema_left.GetColumn(c).GetName() == agg.column_name) {
                            val = (rec.GetValue(c).GetType() == DataType::INT ? rec.GetValue(c).AsInt() : 0); break;
                        }
                }
                groups[group_key][i].count++; groups[group_key][i].sum += val;
    ...
    for (size_t i = 0; i < schema_left.GetColumnCount(); ++i) out << std::setw(15) << schema_left.GetColumn(i).GetName() << (i == schema_left.GetColumnCount() - 1 ? "" : " | ");
...
Status Executor::ExecuteDelete(const DeleteStatement& stmt, std::ostream& out) {
    if (!catalog_.TableExists(stmt.table_name)) return Status::IOError("Table not found: " + stmt.table_name);
    const Schema& schema = catalog_.GetSchema(stmt.table_name);
    TableHeap table(pager_, schema);
    std::vector<std::string> table_names = catalog_.GetTableNames();
    struct DependentFK { std::string table_name; ForeignKey fk; };
    std::vector<DependentFK> dependents;
    for (const auto& t_name : table_names) {
        const auto& s = catalog_.GetSchema(t_name);
        for (const auto& fk : s.GetForeignKeys()) if (fk.referenced_table == stmt.table_name) dependents.push_back({t_name, fk});
    }
    uint32_t deleted_count = 0; Page page;
    for (PageID i = 1; i < pager_.GetPageCount(); ++i) {
        pager_.ReadPage(i, page); auto* header = page.GetHeader();
        if (header->magic != SLOTTED_PAGE_MAGIC) continue;
        bool page_modified = false;
        for (uint32_t s = 0; s < header->num_slots; ++s) {
            Slot* slot = page.GetSlot(s);
            if (!slot->deleted) {
                size_t bytes_read = 0; Record rec = Record::Deserialize(schema, page.GetData() + slot->offset, bytes_read);
                if (Matches(rec, schema, stmt.where.get())) {
                    for (const auto& dep : dependents) {
                        const auto& dep_schema = catalog_.GetSchema(dep.table_name);
                        TableHeap dep_table(pager_, dep_schema); auto dep_records = dep_table.Scan();
                        Value parent_val(0);
                        for (size_t c = 0; c < schema.GetColumnCount(); ++c) if (schema.GetColumn(c).GetName() == dep.fk.referenced_column) parent_val = rec.GetValue(c);
                        for (const auto& dr : dep_records) {
                            for (size_t c = 0; c < dep_schema.GetColumnCount(); ++c) {
                                if (dep_schema.GetColumn(c).GetName() == dep.fk.column_name) {
                                    const auto& dv = dr.GetValue(c); bool match = false;
                                    if (dv.GetType() == DataType::INT && parent_val.GetType() == DataType::INT) match = (dv.AsInt() == parent_val.AsInt());
                                    else match = (dv.AsString() == parent_val.AsString());
                                    if (match) return Status::IOError("Foreign Key constraint violation");
                                }
                            }
                        }
                    }
                    log_manager_.AppendLog(LogRecordType::DELETE, stmt.table_name, Record::Serialize(schema, rec), {});
                    slot->deleted = true; page_modified = true; deleted_count++;
                }
            }
        }
        if (page_modified) table.UpdatePage(i, page);
    }
...
Status Executor::ExecuteUpdate(const UpdateStatement& stmt, std::ostream& out) {
    ...
    const Schema& schema = catalog_.GetSchema(stmt.table_name);
    TableHeap table(pager_, schema);
    uint32_t updated_count = 0; Page page;
    for (PageID i = 1; i < pager_.GetPageCount(); ++i) {
        pager_.ReadPage(i, page); auto* header = page.GetHeader();
        if (header->magic != SLOTTED_PAGE_MAGIC) continue;
        bool page_modified = false;
                ...
                if (Matches(rec, schema, stmt.where.get())) {
                    ...
                    for (size_t c = 0; c < schema.GetColumnCount(); ++c) {
                        if (schema.GetColumn(c).GetName() == stmt.column_name) {
                            if (schema.GetColumn(c).GetType() == DataType::INT) new_values[c] = Value(std::stoi(stmt.new_value));
                            else new_values[c] = Value(stmt.new_value);
                            break;
                        }
                    }
                    ...
                    slot->deleted = true; page_modified = true; table.InsertRecord(updated_rec); updated_count++;
                }
            }
        }
        if (page_modified) table.UpdatePage(i, page);
...
Status Executor::ExecuteCreateIndex(const CreateIndexStatement& stmt, std::ostream& out) {
    ...
    for (const auto& col_name : stmt.column_names) {
        bool found = false;
        for (size_t i = 0; i < schema.GetColumnCount(); ++i) {
            if (schema.GetColumn(i).GetName() == col_name) { col_indices.push_back(i); found = true; break; }
        }
    ...
    TableHeap table(pager_, schema); auto all_records = table.Scan();
    if (type == IndexType::HASH) {
        auto idxs = catalog_.GetHashIndexes(stmt.table_name); auto* idx = idxs.back();
        for (const auto& rec : all_records) { std::vector<Value> keys; for (size_t ci : col_indices) keys.push_back(rec.GetValue(ci)); idx->Insert(keys, rec); }
    } else {
        auto idxs = catalog_.GetBTreeIndexes(stmt.table_name); auto* idx = idxs.back();
        for (const auto& rec : all_records) { std::vector<Value> keys; for (size_t ci : col_indices) keys.push_back(rec.GetValue(ci)); idx->Insert(keys, rec); }
    }
...
Status Executor::ExecuteTransaction(const TransactionStatement& stmt, std::ostream& out) {
    switch (stmt.type) {
        case TransactionType::BEGIN:
            ...
            log_manager_.AppendLog(LogRecordType::BEGIN); in_transaction_ = true; out << "Transaction started" << std::endl; break;
        case TransactionType::COMMIT:
            if (!in_transaction_) return Status::IOError("No transaction in progress");
            log_manager_.AppendLog(LogRecordType::COMMIT); in_transaction_ = false; out << "Transaction committed" << std::endl; break;
        case TransactionType::ROLLBACK:
            if (!in_transaction_) return Status::IOError("No transaction in progress");
            log_manager_.AppendLog(LogRecordType::ROLLBACK); in_transaction_ = false; out << "Transaction rolled back" << std::endl; break;
    }
    return Status::OK();
}

Status Executor::ExecuteExplain(const ExplainStatement& stmt, std::ostream& out) {
    if (!stmt.stmt) return Status::IOError("Explain requires an underlying statement");
    out << "QUERY PLAN:" << std::endl;
    switch (stmt.stmt->GetType()) {
        case StatementType::SELECT: {
            const auto& sel = static_cast<const SelectStatement&>(*stmt.stmt);
            if (!catalog_.TableExists(sel.table_name)) return Status::IOError("Table not found: " + sel.table_name);
            if (sel.join) {
                out << "-> Nested Loop Join (" << sel.table_name << " JOIN " << sel.join->right_table << ")" << std::endl;
                out << "   Condition: " << sel.join->left_table << "." << sel.join->left_column << " = " << sel.join->right_table << "." << sel.join->right_column << std::endl;
            } else {
                bool used_idx = false;
                if (sel.where) {
                    auto b_idxs = catalog_.GetBTreeIndexes(sel.table_name);
                    for (auto* idx : b_idxs) { if (!idx->GetColumnNames().empty() && idx->GetColumnNames()[0] == sel.where->column_name) { out << "-> B-Tree Index Scan [" << idx->GetColumnNames()[0] << "]" << std::endl; used_idx = true; break; } }
                    if (!used_idx && sel.where->op == OpType::EQUAL) {
                        auto h_idxs = catalog_.GetHashIndexes(sel.table_name);
                        for (auto* idx : h_idxs) { if (!idx->GetColumnNames().empty() && idx->GetColumnNames()[0] == sel.where->column_name) { out << "-> Hash Index Lookup [" << idx->GetColumnNames()[0] << "]" << std::endl; used_idx = true; break; } }
                    }
                }
                if (!used_idx) out << "-> Full Table Scan on " << sel.table_name << std::endl;
                if (sel.where) out << "   Filter: " << sel.where->column_name << " " << static_cast<int>(sel.where->op) << " " << sel.where->value << std::endl;
                if (!sel.aggregates.empty()) out << "-> Aggregation (" << sel.aggregates.size() << " functions)" << std::endl;
                if (!sel.group_by_column.empty()) out << "-> Group By: " << sel.group_by_column << std::endl;
            }
            break;
        }
        case StatementType::INSERT: out << "-> Insert into table" << std::endl; break;
        case StatementType::DELETE: out << "-> Delete from table" << std::endl; break;
        case StatementType::UPDATE: out << "-> Update table" << std::endl; break;
        default: out << "-> Generic Statement Execution" << std::endl;
    }
    return Status::OK();
}
